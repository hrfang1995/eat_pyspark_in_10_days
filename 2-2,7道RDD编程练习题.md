...
students = [("LiLei",18,87),("HanMeiMei",16,77),("DaChui",16,66),("Jim",18,77),("RuHua",18,50)]
rdd_data=sc.parallelize(students)
rdd_sorted = rdd_data.map(lambda x: Order(x[0], x[1], x[2]))\\
...
```python
rdd_classes = sc.parallelize(classes).map(lambda x: (x[1], x[0])) #把主键放在第一列
...
rdd_result = rdd_join.groupByKey().map(lambda s: (s[0], average(s[1])))\\
    .filter(lambda z: z[1] > 75)
print(rdd_result)
```
...
rdd_students = sc.paralleliza(students)
rdd_classes = rdd_students.aggregateByKey([], lambda arr,x:arr+[x],
